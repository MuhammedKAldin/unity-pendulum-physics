...
public class SwingController : MonoBehaviour
{
    ...
    void DoSwingAction()
    {
        ...
        {
            //pendulum.bob.velocity += pendulum.bob.velocity.normalized * 0.4f;

            // Navigating In Air
            float horizontalInput = Input.GetAxis("Horizontal");
            float verticalInput = Input.GetAxis("Vertical");

            var movementDirection = cam.transform.forward * verticalInput + cam.transform.right * horizontalInput;

            //movementDirection.Normalize();
            transform.Translate(movementDirection * 3f * Time.deltaTime, Space.World);
        }
        if (Input.GetKey(KeyCode.A))
        {
            pendulum.bob.velocity += -cam.transform.right * Time.deltaTime ;
        }
